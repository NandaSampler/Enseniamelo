...
<<<<<<< HEAD
    ...
    @Operation(summary = "Obtener usuario actual (por token)", 
               description = "Retorna los datos del usuario autenticado a partir del token JWT (sin pasar ID)")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Usuario encontrado"),
        @ApiResponse(responseCode = "404", description = "Usuario no encontrado")
    })
    @GetMapping(value = "/me", produces = "application/json")
    public Mono<ResponseEntity<AuthResponse>> getCurrentUserNoParam(@AuthenticationPrincipal org.springframework.security.oauth2.jwt.Jwt jwt) {
        ...
        if (email != null) {
            return authService.getUserByEmail(email)
                    .map(ResponseEntity::ok)
                    .defaultIfEmpty(ResponseEntity.ok(new AuthResponse(sub, null, jwt.getClaimAsString("preferred_username"), null, email, null, "Usuario obtenido desde token")));
        }
