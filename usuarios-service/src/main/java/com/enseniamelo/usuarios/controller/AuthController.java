...
public class AuthController {
        ...
        public Mono<AuthResponse> register(
                ...
                return authService.register(request)
                                .doOnSuccess(response -> log.info("Usuario registrado exitosamente con id: {}",
                                                response.getId()))
                                .doOnError(error -> log.error("Error registrando usuario {}: {}", request.getEmail(),
                                                error.getMessage()));
        }
        ...
        public Mono<AuthResponse> login(
                ...
                return authService.login(request)
                                ...
                                .doOnError(error -> log.error("Error en login para {}: {}", request.getEmail(),
                                                error.getMessage()));
        }
        ...
        public Mono<ResponseEntity<AuthResponse>> getCurrentUser(
                ...
                return authService.getUserById(userId)
                                ...
                                .doOnError(error -> log.error("Error obteniendo usuario {}: {}", userId,
                                                error.getMessage()));
        }
        ...
        public Mono<ResponseEntity<AuthResponse>> getCurrentUserNoParam(
                ...
                if (email != null) {
                        return authService.getUserByEmail(email)
                                        .map(ResponseEntity::ok)
                                        .defaultIfEmpty(
                                                        ResponseEntity.ok(new AuthResponse(sub, null,
                                                                        jwt.getClaimAsString("preferred_username"),
                                                                        null, email, null,
                                                                        "Usuario obtenido desde token")));
                }

                // If no email in token, return fallback with sub if possible
                AuthResponse fallback = new AuthResponse(sub, null, jwt.getClaimAsString("preferred_username"), null,
                                null,
