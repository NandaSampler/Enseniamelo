# cursoservice/app/api/v1/routers/categoria_router.py
...
def get_categoria(categoria_id: int, service: CategoriaService = Depends(get_categoria_service)):
...
def delete_categoria(categoria_id: int, service: CategoriaService = Depends(get_categoria_service)):
    service.delete(categoria_id)
    return None

# Cursos por categoría (consulta inversa)
@router.get("/{categoria_id}/cursos", response_model=List[CursoOut])
def list_cursos_por_categoria(
    categoria_id: int, cc_service: CursoCategoriaService = Depends(get_cc_service)
):
