// frontend/src/components/Admin/SolicitudesTutores.jsx
...
const mapSolicitudCompletaFromApi = (raw) => {
  ...
  const precioFormateado =
    precioReserva > 0 ? `${precioReserva} Bs/hora` : "Sin precio definido";
  ...
  return {
    ...
    curso: {
      ...
      nombre:
        curso.nombre ||
        curso.titulo ||
        `Curso - ID: ${curso.id || solicitud.idCurso}`,
...
const SolicitudesTutores = () => {
  ...
  const fetchSolicitudes = async () => {
    ...
    try {
      ...
      if (data?.success && Array.isArray(data.solicitudes)) {
        const solicitudesMapeadas = data.solicitudes.map(
          mapSolicitudCompletaFromApi
        );
  ...
  const handleChangeEstado = async (
    ...
    try {
      ...
      if (
        ...
        response = await verificarAPI.aprobarSolicitud(
          id_verificar,
          nuevoComentario || ""
        );
  ...
  const filteredSolicitudes = solicitudes.filter((sol) => {
    ...
    if (
      filterNormalizado === "aceptado" ||
      filterNormalizado === "aprobado"
    ) {
      return (
        estadoNormalizado === "aceptado" ||
        estadoNormalizado === "aprobado"
        ...
        <div className="admin-panel-filters">
          ...
          >
            ...
            {solicitudes.length > 0 &&
              ` (${
                solicitudes.filter(
                  (s) => s.estado?.toLowerCase() === "rechazado"
                ).length
              })`}
          </button>
        </div>
...
export default SolicitudesTutores;