  ...
  const handleSubmit = async (e) => {
    ...
    if (!puedeResenar) {
      showNotification({
        ...
        message:
          mensajeRestriccion ||
          "Debes tener una reserva confirmada para poder dejar una reseña",
      });
      return;
    }
      ...
      try {
        ...
        if (!reservaCurso) {
          ...
          setMensajeRestriccion(
            "Debes reservar y completar este curso antes de poder dejar una reseña"
          );
          return;
        }
        ...
        if (estado === "confirmada") {
          ...
          if (!fechaReserva) {
            setPuedeResenar(false);
            setMensajeRestriccion(
              "La reserva no tiene fecha asignada. Contacta al tutor."
            );
            return;
          }
          ...
          if (ahora < fechaReservaDate) {
            setPuedeResenar(false);
            const fechaFormateada = fechaReservaDate.toLocaleDateString(
              "es-ES",
              {
                day: "2-digit",
                month: "2-digit",
            ...
            setMensajeRestriccion(
              `Podrás dejar una reseña después de tu clase programada para el ${fechaFormateada}`
            );
          } else {
          ...
          if (fechaReserva) {
            ...
            if (ahora < fechaReservaDate) {
              ...
              setMensajeRestriccion(
                `Tu clase está programada para el ${fechaFormateada}. Podrás dejar una reseña después de completarla.`
              );
            } else {
              setMensajeRestriccion(
                "Tu clase ya pasó. Espera a que el tutor la marque como completada para poder dejar tu reseña."
              );
            }
          } else {
            setMensajeRestriccion(
              "Tu reserva está confirmada. Podrás dejar una reseña después de completar la clase."
            );
          }
        } else if (estado === "pendiente") {
          setPuedeResenar(false);
          setMensajeRestriccion(
            "Tu reserva está pendiente de confirmación por el tutor"
          );
        } else if (estado === "cancelada") {
          setPuedeResenar(false);
          setMensajeRestriccion(
            "Tu reserva fue cancelada. No puedes dejar una reseña."
          );
        } else {
          setPuedeResenar(false);
          setMensajeRestriccion(
            "Debes completar el curso antes de poder dejar una reseña"
          );
        }
    ...
    try {
      ...
      if (!cursoId || !usuarioId || !tutorId) {
        showNotification({
          type: "error",
          title: "Error",
          message: "No se pudo identificar usuario o tutor",
        });
        return;
      }
      ...
      const payload = {
        ...
        ultimoMensaje: "Hola, estoy interesado",
      };
