...
const getTutorIdFromCursoRaw = (c) => {
  ...
  // posibles nombres en tu backend
  const candidates = [
    c?.id_tutor,
    c?.idTutor,
    c?.tutor_id,
    ...
    c?.tutor?.id,
    c?.tutor?.$oid,
    c?.tutor, // por si es directamente string/id
  ];
          ...
          {Array.isArray(categoriasNombres) && categoriasNombres.length > 0 ? (
            categoriasNombres.map((cat, index) => (
              <span
                key={`${cat}-${index}`}
                className={
                  "infocurso-chip " + colorClasses[index % colorClasses.length]
                }
              >
                {cat}
              </span>
        ...
        {tieneCupo && cupoTotalInt !== null && (
          <div className="infocurso-cupos-info">
            <p className="infocurso-cupos-disponibles">
              ...
              {cuposDisponiblesCalc !== null
                ? ` · Disponibles: ${cuposDisponiblesCalc}`
                : ""}
            </p>
          </div>
        )}

        {/* ✅ Disponibilidad (del endpoint de reservas) */}
  ...
  const handleSubmit = async (e) => {
    ...
    if (!puedeResenar || yaReseno) {
      showNotification({
        ...
        message:
          "Solo puedes dejar una reseña por curso después de tomar la clase",
      });
      return;
    }
    ...
    <section className="infocurso-reviews-section">
          ...
          {comentarios.length === 0 ? (
            ...
            <div className="infocurso-reviews-grid">
              {comentarios.map((comentario) => (
                <ReviewCard
                  key={
                    comentario.idComentario || comentario._id || comentario.id
                  }
      ...
      {puedeResenar && !yaReseno ? (
        ...
        <p className="infocurso-review-hint">
          Solo puedes dejar una reseña por curso y únicamente cuando tu reserva
          haya sido confirmada y la fecha de la clase ya haya pasado.
        </p>
      )}
    </section>
  );
...
const InfoCurso = () => {
  ...
  // ✅ Guardamos el tutorId real para crear el chat correctamente
  ...
  useEffect(() => {
    const fetchCurso = async () => {
      ...
      try {
        ...
        console.log("Tutor ID detectado del curso:", tutorIdDetectado);
  ...
  useEffect(() => {
    const fetchDisponibilidad = async () => {
      ...
      try {
        ...
        const { data } = await reservasAPI.verificarDisponibilidad(
          id,
          usuarioId
        );
  ...
  useEffect(() => {
    const checkReservaParaResena = async () => {
      ...
      try {
        ...
        const reservas = Array.isArray(raw)
          ? raw
        ...
        const tieneReservaCurso = reservas.some((r) => {
          const rCurso = r?.id_curso;
          const rCursoId =
            typeof rCurso === "string" ? rCurso : rCurso?._id || rCurso?.id;
          return String(rCursoId) === String(id) && r?.estado !== "cancelada";
        });

        const tieneReservaValida = reservas.some((r) => {
          const rCurso = r?.id_curso;
          const rCursoId =
            typeof rCurso === "string" ? rCurso : rCurso?._id || rCurso?.id;
          if (String(rCursoId) !== String(id)) return false;
          if (r?.estado !== "confirmada" && r?.estado !== "completada")
            return false;

          const fechaClase = r?.fecha ? new Date(r.fecha) : null;
        ...
        const yaComentado = comentarios.some(
          (c) => String(c?.id_usuario) === String(usuarioId)
        );
  ...
  const handleOpenChat = async () => {
    try {
      ...
      if (!cursoId || !usuarioId || !tutorId) {
        showNotification({
          type: "error",
          title: "Error",
          message: "No se pudo identificar a los participantes del chat",
        });
        return;
      }
      ...
      const payload = {
        idCurso: cursoId,
        participantes: [usuarioId, tutorId],
        ultimoMensaje: "",
      };

      const resp = await chatsAPI.createChat(payload);
      const d = resp?.data;
      ...
      if (chatId) {
        // ✅ GUARDA cursoId para este chat (fallback si backend devuelve id_curso null)
        localStorage.setItem(`chatCurso:${chatId}`, String(cursoId));

        // ✅ pasa cursoId por navigation state también
  ...
  const handleReservar = async () => {
    try {
      ...
      // ✅ aceptar {success:true} o ReservaOut directa (con id)
      ...
      if (!ok) {
        showNotification({
          ...
          message:
            body?.message ||
            body?.detail ||
            "No se pudo crear la reserva para este curso.",
        });
        return;
      }
      ...
      showNotification({
        ...
        message:
          "Tu reserva fue creada. Ahora espera a que el tutor la acepte.",
      });

      // ✅ abrir chat creado correctamente (con id_curso y participantes)
      await handleOpenChat();
    } catch (error) {
  ...
  const handleAddReview = async (reviewData) => {
    try {
      ...
      showNotification({
        ...
        title: "Error",
        message:
          error?.response?.data?.message || "No se pudo publicar la reseña",
      });
    }
  };
